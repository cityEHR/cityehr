...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Match the root node and call template to generate the ontology -->
    <xsl:template match="/">
        ...
        <xsl:if test="$processError='true'">
            <xsl:call-template name="generateOWLError">
                ...
                <xsl:with-param name="message">applicationId, specialtyId, classId and base language must be
                    defined in the model.</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
