...
<xsl:stylesheet exclude-result-prefixes="xs" version="2.0" xmlns:xf="http://www.w3.org/2002/xforms" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- Get the template XML content for the ODF document
         This has already been extracted from the .odt template and placed in the templates directory in the standard configuration.
         Only used for testing purposes -->
    <xsl:variable name="odfTemplate" select="document('../resources/templates/letterTemplate-content.xml')/office:document-content"/>
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="filename" as="xs:string" select="concat('../existSandBox/',$applicationLocation,'/ODF/',$specialtyLocation,'/',$compositionLocation,'.xml')"/>
