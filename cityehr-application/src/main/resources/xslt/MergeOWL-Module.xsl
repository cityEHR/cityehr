...
<!-- ====================================================================
    ...
    The base ontology for the merge is the standard document input to the transformation
    All assertions from the base ontology are copied through to the output
    The $merge input holds the ontology to be merged into the base
    Any assertions in $merge that are not in the base are added to the output
...
<xsl:stylesheet exclude-result-prefixes="xs" version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="mergeRootNode" select="$merge/descendant::owl:Ontology[1]"/>
...
-->
        <Ontology xmlns:office2003Spreadsheet="urn:schemas-microsoft-com:office:spreadsheet"
            xmlns:x2="http://schemas.microsoft.com/office/excel/2003/xml" xmlns="http://www.w3.org/2002/07/owl#"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:cda="urn:hl7-org:v3" xmlns:owl="http://www.w3.org/2002/07/owl#"
            xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:c="urn:schemas-microsoft-com:office:component:spreadsheet"
            xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:html="http://www.w3.org/TR/REC-html40"
            xmlns:xsd="http://www.w3.org/2001/XMLSchema#" xmlns:o="urn:schemas-microsoft-com:office:office"
            xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"
    ...
    <xsl:template name="mergeOWLAssertions">
        ...
        <xsl:for-each-group select="$mergeRootNode/owl:InverseObjectProperties" group-by="*[1]/@IRI">
            <xsl:for-each-group select="current-group()" group-by="*[2]/@IRI">
                <xsl:if test="empty(key('InverseObjectPropertiesList', concat(*[1]/@IRI, *[2]/@IRI), $rootNode))">
                    <xsl:copy-of select="." copy-namespaces="no"/>
                </xsl:if>
            </xsl:for-each-group>
        </xsl:for-each-group>
        ...
        <xsl:for-each-group select="$mergeRootNode/owl:SubDataPropertyOf" group-by="*[1]/@IRI">
            <xsl:for-each-group select="current-group()" group-by="*[2]/@IRI">
                <xsl:if test="empty(key('SubDataPropertyOfList', concat(*[1]/@IRI, *[2]/@IRI), $rootNode))">
                    <xsl:copy-of select="." copy-namespaces="no"/>
                </xsl:if>
            </xsl:for-each-group>
        </xsl:for-each-group>
        ...
        <xsl:for-each-group select="$mergeRootNode/owl:ClassAssertion" group-by="*[1]/@IRI">
            <xsl:for-each-group select="current-group()" group-by="*[2]/@IRI">
                <xsl:if test="empty(key('ClassAssertionList', concat(*[1]/@IRI, *[2]/@IRI), $rootNode))">
                    <xsl:copy-of select="." copy-namespaces="no"/>
                </xsl:if>
            </xsl:for-each-group>
        </xsl:for-each-group>
        ...
        <!-- ObjectPropertyAssertion -->
        <xsl:for-each-group select="$mergeRootNode/owl:ObjectPropertyAssertion" group-by="*[1]/@IRI">
            <xsl:for-each-group select="current-group()" group-by="*[2]/@IRI">
                <xsl:for-each-group select="current-group()" group-by="*[3]/@IRI">
                    <xsl:if test="empty(key('ObjectPropertyAssertionList', concat(*[1]/@IRI, *[2]/@IRI, *[3]/@IRI), $rootNode))">
                        <xsl:copy-of select="." copy-namespaces="no"/>
                    </xsl:if>
                </xsl:for-each-group>
            </xsl:for-each-group>
        ...
        <xsl:for-each-group select="$mergeRootNode/owl:DataPropertyAssertion" group-by="*[1]/@IRI">
            <xsl:for-each-group select="current-group()" group-by="*[2]/@IRI">
                <xsl:if test="empty(key('DataPropertyAssertionList', concat(*[1]/@IRI, *[2]/@IRI), $rootNode))">
                    <xsl:copy-of select="." copy-namespaces="no"/>
                </xsl:if>
            </xsl:for-each-group>
        </xsl:for-each-group>
